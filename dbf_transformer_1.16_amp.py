...
def DBF(raw_data):
    ...
    color = plt.get_cmap("jet")((amp_copy - np.nanmin(amp_copy)) /
                                (np.nanmax(amp_copy) - np.nanmin(amp_copy)))
    ...
    fig.colorbar(im,
                 format=ticker.FuncFormatter(lambda x, pos: round(
                     x * (max_amp - min_amp) + min_amp, 1))).set_label(
                         'amp [dB]',
                         loc='top',
                         rotation=0,
                         fontsize=all_font,
                         labelpad=90)  # x:0~1, lambda:min_amp~max_amp
