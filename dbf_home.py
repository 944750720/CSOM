...
def DBF(raw_data):
    ...
    # c = light_speed
    phase_data = np.angle(raw_data[:,:,0:120])
    ...
    theta0 = np.angle(raw_data[:,:,0:120]) # target beam angle
    ...
    azimuth_one_row = np.arange(0, 5, dx) + dx / 2
    azimuth = np.repeat(azimuth_one_row, 120)
    slant_range_one_col = np.arange(0, 120) * dy + dy / 2
    ...
    amp = np.reshape(20*np.log10(np.abs(raw_data[0,:,0:120])), (az_len * rg_e_index))
    ...
    amp[(amp < 8)] = None # set the amp of the point which is too low to None for eliminating noise
    mask = ~np.isnan(amp) # mask the point which is None
    ...
    height = height[mask]
    amp = amp[mask]
    color = plt.get_cmap("jet")(amp / np.nanmax(amp))
    im = ax.scatter(azimuth, ground_range, height, amp, s=5,c=color,marker='.')
    # 2.1 增加侧边colorbar
