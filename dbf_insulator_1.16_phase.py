...
def DBF(raw_data):
    ...
    color = plt.get_cmap("jet")((phase_copy-np.nanmin(phase_copy)) / (np.nanmax(phase_copy)-np.nanmin(phase_copy)))
    im = ax.scatter(azimuth_flatten, ground_range, height, phase_copy, s=50,c=color,marker='.')
    # 2.1 增加侧边colorbar
    ...
    fig.colorbar(im, format=ticker.FuncFormatter(lambda x, pos: round(x*(max_phase-min_phase)+min_phase, 1))).set_label('phase [rad]', loc = 'top', rotation = 0, fontsize = all_font, labelpad = 140) # x:0~1, lambda:min_amp~max_amp
